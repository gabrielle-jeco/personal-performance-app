...
const MobileChecklist: React.FC<MobileChecklistProps> = ({ supervisor, onNavigate }) => {
    ...
    const isFutureDate = (date: Date) => {
        // Reset time part of today for accurate comparison if needed, or simply compare timestamps
        // But simpler:
        // If year > current: true
        // If year == current:
        //    If month > current: true
        //    If month == current: if date > todayDate: true

        // Let's rely on date object comparison but ensure both are "noon" or "start of day" to avoid time issues?
        // Or strictly:
                    ...
                    {myTasks.map(task => (
                        <div key={task.task_id} className="bg-gray-100/50 rounded-2xl p-4 flex items-center justify-between group border border-transparent hover:border-gray-200 transition">
                            ...
                            <div className="flex flex-col gap-2">
                                <button
                                    onClick={() => handleCameraClick(task)}
                                ...
                                >
                                    <Camera size={14} />
                                    {uploadingTaskId === task.task_id ? '...' : (task.proof_image ? 'Foto' : 'Upload')}
                                </button>
                            </div>
                        </div>
                    ))}
